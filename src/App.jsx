      ...
      <GC>
        ...
        <div style={{ padding: 22, display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          {te && hls.length > 0 ? <div><Lbl c={V.orange}>Bildtexte (Deutsch)</Lbl>
            ...
            </div>; })}<div style={{ position: "relative", marginTop: 8 }}><button onClick={() => setAddFmtOpen(!addFmtOpen)} style={{ padding: "6px 12px", borderRadius: 8, border: `1px dashed ${V.teal}40`, background: addFmtOpen ? `${V.teal}08` : "transparent", color: V.teal, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: FN, width: "100%" }}>+ Textbaustein hinzufügen</button>{addFmtOpen && <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 20, marginTop: 4, background: "rgba(255,255,255,0.95)", backdropFilter: "blur(12px)", borderRadius: 12, border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 8px 30px rgba(0,0,0,0.12)", padding: 8, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>{Object.keys(formatLabels).map(f => { const c = formatColors[f]; return <button key={f} onClick={() => { onEditText(sel, "add_bullet", bullets.length, f); setAddFmtOpen(false); }} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, border: `1px solid ${c}20`, background: `${c}04`, cursor: "pointer", fontFamily: FN, textAlign: "left" }}><div style={{ flexShrink: 0, width: 48, height: 30, display: "flex", alignItems: "center", justifyContent: "center" }}>{formatWireframes[f]?.(c) || null}</div><div><div style={{ fontSize: 9, fontWeight: 800, color: c, textTransform: "uppercase", letterSpacing: ".04em" }}>{formatLabels[f]}</div><div style={{ fontSize: 8.5, color: V.textDim, lineHeight: 1.3, marginTop: 1 }}>{formatDescriptions[f]?.split("—")[0]?.trim() || ""}</div></div></button>; })}</div>}</div></div>}
